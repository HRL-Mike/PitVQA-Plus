...
class Vector_MoLoRA(nn.Module):
    def __init__(self, base_layer, lora_rank, alpha, mora_rank, dropout):
        ...
        self.mora_A = nn.ModuleDict({
            'default': nn.Conv1d(self.mora_r, self.mora_r, bias=False, kernel_size=1)  # use Conv1d instead of FC layer
        })
        # zero init for mora_A
        nn.init.zeros_(self.mora_A['default'].weight)
        self.mora_B = self.mora_A 
    ...
    def forward(self, x): 
        ...
        # MoRA branch
        # apply compression before MoRA: RoPE
        ...
        # apply decompression after MoRA
